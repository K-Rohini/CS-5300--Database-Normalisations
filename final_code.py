...
# Function to identify and handle join dependencies for 5NF
def identify_join_dependencies(df):
    # Decompose based on join dependencies
    
    # Relation R1 (DrinkID, Milk)
    R1 = df[['DrinkID', 'Milk']].drop_duplicates()
    print("R1 (DrinkID, Milk):")
    print(R1)
    
    # Relation R2 (OrderID, CustomerID, DrinkID)
    R2 = df[['OrderID', 'CustomerID', 'DrinkID']].drop_duplicates()
    print("\nR2 (OrderID, CustomerID, DrinkID):")
    print(R2)
    
    # Saving decomposed tables as separate CSV files or dataframes
    R1.to_csv("R1.csv", index=False)
    R2.to_csv("R2.csv", index=False)
    
    # Alternatively, return them if needed in code as DataFrames
    return R1, R2

# Function to test for join dependency satisfaction (lossless join test)
def test_5NF_join_dependencies(R1, R2, original_df):
    # Join R1 and R2 on 'DrinkID' to test for a lossless join
    reconstructed_df = pd.merge(R2, R1, on='DrinkID', how='inner')
    print("\nReconstructed Table by joining R1 and R2:")
    print(reconstructed_df)

    # Check if the reconstructed table matches the original data
    if reconstructed_df.equals(original_df):
        print("\n5NF decomposition is successful; the join is lossless.")
    else:
        print("\n5NF decomposition is not lossless.")

# Example input based on your provided table data
data = {
    'OrderID': [1001, 1001, 1002, 1003, 1003, 1003],
...
# Perform 5NF decomposition
R1, R2 = identify_join_dependencies(original_df)

# Test the decomposition for lossless join
...
def convert_to_5NF(tables, JD, primaryKey):
    ...
    for jd in JD:
        ...
        # Check if the join dependency is trivial or lossless
        for tname, cols in tables.items():
            if is_lossless_join(cols, projections, primaryKey):
                # Create new tables for non-trivial JDs
                for proj in projections:
                    newTables[f"Table_{count}"] = list(proj)
                    count += 1
                break
        else:
            print(f"Warning: Join dependency {jd} is not lossless and will be ignored.")

    # Keep remaining attributes in the original table if any
    if not newTables:
        newTables = tables
    else:
        all_new_attrs = set().union(*[set(t) for t in newTables.values()])
        for tname, cols in tables.items():
            remaining_cols = set(cols) - all_new_attrs
            if remaining_cols:
                newTables[f"Table_{count}"] = list(remaining_cols)
                count += 1
...
def generate_sql_queries(FD, Key, tables, data_types):
    ...
    for tname, col in tables.items():
        ...
        fkey = ""
        column_definitions = []  # List to collect column definitions

        for c in col:
            if data_types[c] == "INT":
                if c in flag_values.keys():
                    fkey += f', FOREIGN KEY ({c}) REFERENCES {flag_values[c]}({c})'
                else:
                    flag_values[c] = tname
                    column_definitions.append(f'{c} INT PRIMARY KEY')
            else:
                if data_types[c] == "VARCHAR(100)":
                    column_definitions.append(f'{c} VARCHAR(100)')
                elif data_types[c] == "VARCHAR(50)":
                    column_definitions.append(f'{c} VARCHAR(50)')
                else:
                    column_definitions.append(f'{c} DATE')
                
                if c in Key:
                    column_definitions[-1] += " NOT NULL"  # Modify the last added column definition

        # Join the column definitions with commas and add to the query
        s += ", ".join(column_definitions) + f"{fkey})"  # Add foreign keys after column definitions
        queries.append(s)
